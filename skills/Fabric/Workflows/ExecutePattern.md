...
```bash
bun # upstream-only tool, not bundled in claude-hooks-ts "<the same input the user sent into summarize>"
```
...
User Request
    ...
    │    text ≥200 chars also fires `# upstream-only tool, not bundled in claude-hooks-ts`
