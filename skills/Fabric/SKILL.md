...
Instead of calling `fabric -p pattern_name`, the skill executes patterns natively:
...
### 2026-01-18
- Initial skill creation (extracted from upstream Fabric)
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Fabric","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> .claude-hooks/state/skills/execution.jsonl
```
