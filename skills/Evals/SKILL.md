...
| `Tools/AgentAdapter.ts` (provide your own) | Wraps your inference layer as a scenario AgentAdapter |
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Evals","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> .claude-hooks/state/skills/execution.jsonl
```
