...
export const ElicitationsTest = (initial: ReadonlyArray<ElicitationRecord> = []): Layer.Layer<Elicitations> => Layer.effect(Elicitations, Effect.gen(function* () {
  ...
  return Elicitations.of({
    ...
    gc: (cwd, now, maxAgeMs = DEFAULT_GC_MAX_AGE_MS) => Ref.update(ref, (arr) => { const cutoff = now - maxAgeMs; return arr.filter((r) => r.cwd !== cwd || r.ts >= cutoff) }),
  })
}))
