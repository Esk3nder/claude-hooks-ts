/**
 * Two-step prompt classifier — mirrors PAI's PromptProcessing.hook.ts
 * pre-inference fast-path gates (lines 100-131, 871-914).
 *
 * Step 1 — deterministic fast-path. PORTED VERBATIM from PAI:
 ...
 *
 * What is NOT a fast-path (mirroring PAI doctrine):
...
// ════════════════════════════════════════════════════════════════
// PORTED VERBATIM FROM PAI hooks/PromptProcessing.hook.ts
// ════════════════════════════════════════════════════════════════
...
/** PAI lines 101-113 — ported verbatim. */
...
/** PAI lines 883-897 — praise detection on normalized prompt. */
...
/**
 * PAI line 900 — system-injected text.
 *
 * Exported (renamed `isSystemTextPrompt`) so the prompt-router can check it
 * BEFORE calling classify(). PAI hook handles this with `process.exit(0)`
...
// ════════════════════════════════════════════════════════════════
// END PAI VERBATIM PORT
// ════════════════════════════════════════════════════════════════

/**
 * Return a Classification synchronously when the prompt's shape forces the
 * answer per PAI's pre-inference gates. Returns null for anything ambiguous
 * → caller delegates to Inference.
 *
 * Order matches PAI lines 871-914: rating → praise → system-text → length.
 */
export const tryFastPath = (
  rawPrompt: string,
): Classification | null => {
  const prompt = rawPrompt ?? ""
  // Gate 1: explicit rating (PAI line 872)
  ...
  // Gate 2: positive praise (PAI line 886)
  ...
  // is called (mirrors PAI's process.exit(0) — no additionalContext at all).
  ...
  //
  // Gate 4: short prompt (PAI line 905)
 ...
 * PAI line 58-64 (emitAdditionalContext) byte-for-byte:
 *   ALGORITHM with tier:  "MODE: ALGORITHM | TIER: E${tier} | REASON: ... | SOURCE: classifier"
 *   else:                 "MODE: ${mode} | REASON: ... | SOURCE: classifier"
 *
 * SOURCE in additionalContext collapses fast-path → "classifier" (PAI hard-
 ...
 * telemetry only (PAI line 877-879 vs line 1023) — that's what auditors
