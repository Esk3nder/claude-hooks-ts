/**
 * Two-step prompt classifier — mirrors the upstream classifier doctrine
 * pre-inference fast-path gates (lines 100-131, 871-914).
 *
 * Step 1 — deterministic fast-path. PORTED VERBATIM from the spec:
 ...
 *
 * What is NOT a fast-path (mirroring Algorithm doctrine):
 *   - Single-token "ok" / "yes" / "no" with no further context → goes to
 *     inference, because doctrine v6.3.0 line 749: "single-word approvals
 *     to multi-step plans are NEVER MINIMAL — they inherit the proposal's
 *     mode and tier." Only the Sonnet classifier sees the conversation
...
// ════════════════════════════════════════════════════════════════
// PORTED VERBATIM FROM the upstream hooks/PromptProcessing.hook.ts
// ════════════════════════════════════════════════════════════════
...
/** the upstream classifier — ported verbatim. */
...
/** the upstream classifier — praise detection on normalized prompt. */
...
/**
 * the upstream classifier — system-injected text.
 *
 * Exported (renamed `isSystemTextPrompt`) so the prompt-router can check it
 * BEFORE calling classify(). The upstream hook handles this with `process.exit(0)`
 * (line 901-902) — emits NO additionalContext at all. The router mirrors
...
// ════════════════════════════════════════════════════════════════
// END the upstream spec VERBATIM PORT
// ════════════════════════════════════════════════════════════════

/**
 * Return a Classification synchronously when the prompt's shape forces the
 * answer per the upstream spec's pre-inference gates. Returns null for anything ambiguous
 * → caller delegates to Inference.
 *
 * Order matches the upstream classifier: rating → praise → system-text → length.
 */
export const tryFastPath = (
  ...
  // is called (mirrors the upstream process.exit(0) — no additionalContext at all).
...
export interface ClassifyOptions {
   ...
   *  inside Inference. This is what makes the doctrine rule "single-word
 ...
 * the upstream classifier (emitAdditionalContext) byte-for-byte:
 *   ALGORITHM with tier:  "MODE: ALGORITHM | TIER: E${tier} | REASON: ... | SOURCE: classifier"
 *   else:                 "MODE: ${mode} | REASON: ... | SOURCE: classifier"
 *
 * SOURCE in additionalContext collapses fast-path → "classifier" (the upstream spec hard-
 ...
 * Algorithm v6.3.0 line 79. The "fast-path" distinction is preserved in
 * telemetry only (the upstream classifier vs line 1023) — that's what auditors
