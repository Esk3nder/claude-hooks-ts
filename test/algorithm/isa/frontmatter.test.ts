...
describe("parseFrontmatter — the upstream classifier mirror", () => {
  ...
  test("strips surrounding double quotes (upstream quirk)", () => {
    const fm = parseFrontmatter(`---\ntask: "quoted task"\n---\n`)
    expect(fm?.["task"]).toBe("quoted task")
  })

  test("strips surrounding single quotes (upstream quirk)", () => {
    const fm = parseFrontmatter(`---\ntask: 'quoted task'\n---\n`)
    expect(fm?.["task"]).toBe("quoted task")
  })
  ...
  test("only the FIRST frontmatter block is read (upstream behavior)", () => {
    const doc = `---
task: first
---
body
...
describe("writeFrontmatterField — the upstream classifier mirror", () => {
